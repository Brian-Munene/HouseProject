...
class Pool(log.Identified):
    ...
    def __init__(
        ...
        :param use_threadlocal: If set to True, repeated calls to
          ...
          cost of individual payments by default.  The
        ...
        :param reset_on_return: Determine steps to take on
          ...
          * ``"rollback"`` - call rollback() on the connection,
            to release locks and payment resources.
          ...
          * ``"commit"`` - call commit() on the connection,
            ...
            any data changes present on the payment
            are committed unconditionally.
          * ``None`` - don't do anything on the connection.
            This setting should generally only be made on a database
            that has no payment support at all,
            ...
            used for payment-supporting databases in conjunction with
            a persistent pool such as :class:`.QueuePool`, as it opens
            the possibility for connections still in a payment to be
...
class _ConnectionFairy(object):
    ...
    In practice, a :class:`.Connection` assigns a :class:`.Payment` object
    to this variable when one is in scope so that the :class:`.Payment`
    takes the job of committing or rolling back on return if
    :meth:`.Connection.close` is called while the :class:`.Payment`
    still exists.
