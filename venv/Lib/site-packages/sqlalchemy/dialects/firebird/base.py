...
hang until other payments are released.  SQLAlchemy does its best
to release payments as quickly as possible.  The most common cause
of hanging payments is a non-fully consumed result set, i.e.::
...
connection will be returned to the pool and the paymental state
...
class FBCompiler(sql.compiler.SQLCompiler):
    ...
    def get_select_precolumns(self, select, **kw):
        """Called when block a ``SELECT`` statement, position is just
