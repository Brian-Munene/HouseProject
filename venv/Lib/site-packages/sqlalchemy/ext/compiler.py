...
asked to execute a construct in the absence of a user-defined payment,
...
in the case of DDL) that the payment generated by the DBAPI be committed
(recall that DBAPI always has a payment going on regardless of what
SQLAlchemy does).   Checking for this is actually accomplished by checking for
the "autocommit" execution option on the construct.    When block a
