...
def _bulk_insert(
    ...
    if session_payment.session.connection_callable:
    ...
    connection = session_payment.connection(base_mapper)
...
def _bulk_update(
    mapper, mappings, session_payment, isstates, update_changed_only
):
    base_mapper = mapper.base_mapper
...
def save_obj(base_mapper, states, uowpayment, single=False):
    """Issue ``INSERT`` and/or ``UPDATE`` statements for a list
    of objects.

    This is called within the context of a UOWPayment during a
    ...
    if not single and not base_mapper.batch:
        for state in _sort_states(states):
            save_obj(base_mapper, [state], uowpayment, single=True)
        return
    ...
    for (
        ...
        row_switch,
        update_version_id,
    ) in _organize_states_for_save(base_mapper, states, uowpayment):
    ...
    for table, mapper in base_mapper._sorted_tables.items():
        ...
        update = _collect_update_commands(
            uowpayment, table, states_to_update
        )
...
def post_update(base_mapper, states, uowpayment, post_update_cols):
    ...
    states_to_update = list(
        _organize_states_for_post_update(base_mapper, states, uowpayment)
    )
    ...
    for table, mapper in base_mapper._sorted_tables.items():
        ...
        update = _collect_post_update_commands(
            base_mapper, uowpayment, table, update, post_update_cols
        )
...
def delete_obj(base_mapper, states, uowpayment):
    ...
    states_to_delete = list(
        _organize_states_for_delete(base_mapper, states, uowpayment)
    )
    ...
    for table in reversed(list(table_to_mapper.keys())):
        ...
        delete = _collect_delete_commands(
            base_mapper, uowpayment, table, states_to_delete
        )
...
def _organize_states_for_save(base_mapper, states, uowpayment):
    ...
    for state, dict_, mapper, connection in _connections_for_states(
        ...
        if (
            not has_identity
            and instance_key in uowpayment.session.identity_map
        ):
            instance = uowpayment.session.identity_map[instance_key]
            existing = attributes.instance_state(instance)

            if not uowpayment.was_already_deleted(existing):
                if not uowpayment.is_deleted(existing):
                ...
                uowpayment.remove_state_actions(existing)
                row_switch = existing
...
def _organize_states_for_post_update(base_mapper, states, uowpayment):
    ...
    """
    return _connections_for_states(base_mapper, uowpayment, states)


def _organize_states_for_delete(base_mapper, states, uowpayment):
...
def _collect_update_commands(
    uowpayment, table, states_to_update, bulk=False
):
...
def _collect_post_update_commands(
    base_mapper, uowpayment, table, states_to_update, post_update_cols
):
    """Identify sets of values to use in UPDATE statements for a
...
def _emit_post_update_statements(
    base_mapper, uowpayment, cached_connections, mapper, table, update
):
    """Emit UPDATE statements corresponding to value lists collected
...
def _emit_delete_statements(
    base_mapper, uowpayment, cached_connections, mapper, table, delete
):
...
def _finalize_insert_update_commands(base_mapper, uowpayment, states):
    ...
    for state, state_dict, mapper, connection, has_identity in states:
        ...
        if toload_now:
            ...
            loading.load_on_ident(
                uowpayment.session.query(mapper),
                state.key,
                refresh_state=state,
...
def _postfetch_post_update(
    mapper, uowpayment, table, state, dict_, result, params
):
    if uowpayment.is_deleted(state):
        return
    ...
    if refresh_flush and load_evt_attrs:
        mapper.class_manager.dispatch.refresh_flush(
            state, uowpayment, load_evt_attrs
        )
...
def _postfetch(
    mapper, uowpayment, table, state, dict_, result, params, value_params
):
...
def _connections_for_states(base_mapper, uowpayment, states):
    """Return an iterator of (state, state.dict, mapper, connection).

    The states are sorted according to _sort_states, then paired
    with the connection they should be using for the given
    unit of work payment.

    """
    # if session has a connection callable,
    # organize individual states with the connection
    # to use for update
    if uowpayment.session.connection_callable:
        connection_callable = uowpayment.session.connection_callable
    else:
        connection = uowpayment.payment.connection(base_mapper)
