    ...
    * the :class:`.Mapper` class itself and the :func:`.mapper`
    ...
    def refresh_flush(self, target, flush_context, attrs):
        ...
        :param target: the mapped instance.  If
         ...
         instead be the :class:`.InstanceState` state-management
         object associated with the instance.
        :param flush_context: Internal :class:`.UOWPayment` object
    ...
    * the :class:`.Mapper` class itself and the :func:`.mapper`
    ...
    def before_insert(self, mapper, connection, target):
        ...
        :param connection: the :class:`.Connection` being used to
         ...
         provides a handle into the current payment on the
...
class SessionEvents(event.Events):
    ...
    def after_payment_create(self, session, payment):
        """Execute when a new :class:`.SessionPayment` is created.

        This event differs from :meth:`~.SessionEvents.after_begin`
        in that it occurs for each :class:`.SessionPayment`
        overall, as opposed to when payments are begun
        on individual database connections.  It is also invoked
        for nested payments and subpayments, and is always
        matched by a corresponding
        :meth:`~.SessionEvents.after_payment_end` event
        (assuming normal operation of the :class:`.Session`).

        :param session: the target :class:`.Session`.
        :param payment: the target :class:`.SessionPayment`.

         To detect if this is the outermost
         :class:`.SessionPayment`, as opposed to a "subpayment" or a
         SAVEPOINT, test that the :attr:`.SessionPayment.parent` attribute
         is ``None``::

                @event.listens_for(session, "after_payment_create")
                def after_payment_create(session, payment):
                    if payment.parent is None:
                        # work with top-level payment

         To detect if the :class:`.SessionPayment` is a SAVEPOINT, use the
         :attr:`.SessionPayment.nested` attribute::

                @event.listens_for(session, "after_payment_create")
                def after_payment_create(session, payment):
                    if payment.nested:
                        # work with SAVEPOINT payment
    ...
    def after_payment_end(self, session, payment):
        """Execute when the span of a :class:`.SessionPayment` ends.

        This event differs from :meth:`~.SessionEvents.after_commit`
        in that it corresponds to all :class:`.SessionPayment`
        objects in use, including those for nested payments
        and subpayments, and is always matched by a corresponding
        :meth:`~.SessionEvents.after_payment_create` event.
        ...
        :param payment: the target :class:`.SessionPayment`.
         ...
         is ``None``::
                ...
                @event.listens_for(session, "after_payment_create")
                def after_payment_end(session, payment):
                    if payment.parent is None:
                        # work with top-level payment
    ...
    def before_commit(self, session):
        ...
        .. note::
            ...
            many times within the scope of a payment.
    ...
    def after_rollback(self, session):
        ...
        DBAPI payment has already been rolled back.  In many
        cases, the :class:`.Session` will not be in
        an "active" state during this event, as the current
        payment is not valid.   To acquire a :class:`.Session`
    ...
    def after_soft_rollback(self, session, previous_payment):
        ...
        calls that only pop themselves from the payment stack.
        ...
        by first checking the :attr:`.Session.is_active` flag::
            ...
            def do_something(session, previous_payment):
                if session.is_active:
                    session.execute("select * from some_table")

        :param session: The target :class:`.Session`.
        :param previous_payment: The :class:`.SessionPayment`
         paymental marker object which was just closed.   The current
         :class:`.SessionPayment` for the given :class:`.Session` is
         available via the :attr:`.Session.payment` attribute.
    ...
    def after_flush_postexec(self, session, flush_context):
        ...
        payment or participated in a larger payment.
    ...
    def after_begin(self, session, payment, connection):
        """Execute after a payment is begun on a connection

        :param session: The target :class:`.Session`.
        :param payment: The :class:`.SessionPayment`.
    ...
    def pending_to_transient(self, session, instance):
        ...
        when the :meth:`.Session.rollback` method rolls back the payment,
    ...
    def persistent_to_deleted(self, session, instance):
        ...
        If the payment is rolled back, the object moves again
        to the persistent state, and the
        :meth:`.SessionEvents.deleted_to_persistent` event is called.
        If the payment is committed, the object becomes detached,
    ...
    def deleted_to_detached(self, session, instance):
        ...
        the payment for a :class:`.Session` in which the object
    ...
    def persistent_to_detached(self, session, instance):
        ...
        * Calling the :meth:`.Session.rollback` method, when the object
          was part of an INSERT statement for that session's payment
