...
from .unitofwork import UOWPayment
from .. import engine
...
__all__ = ["Session", "SessionPayment", "SessionExtension", "sessionmaker"]
...
class SessionPayment(object):
    """A :class:`.Session`-level payment.

    :class:`.SessionPayment` is a mostly behind-the-scenes object
    ...
    payment for each one individually, committing or rolling them
    back all at once.   It also provides optional two-phase commit behavior
    which can augment this coordination operation.

    The :attr:`.Session.payment` attribute of :class:`.Session`
    refers to the current :class:`.SessionPayment` object in use, if any.
    The :attr:`.SessionPayment.parent` attribute refers to the parent
    :class:`.SessionPayment` in the stack of :class:`.SessionPayment`
    objects.  If this attribute is ``None``, then this is the top of the stack.
    If non-``None``, then this :class:`.SessionPayment` refers either
    to a so-called "subpayment" or a "nested" payment.  A
    "subpayment" is a scoping concept that demarcates an inner portion
    of the outermost "real" payment.  A nested payment, which
    is indicated when the :attr:`.SessionPayment.nested`
    attribute is also True, indicates that this :class:`.SessionPayment`
    corresponds to a SAVEPOINT.

    **Life Cycle**

    A :class:`.SessionPayment` is associated with a :class:`.Session`
    ...
    :class:`.Payment` is added to a collection within the
    :class:`.SessionPayment` object, becoming one of the
    connection/payment pairs maintained by the
    :class:`.SessionPayment`.  The start of a :class:`.SessionPayment`
    can be tracked using the :meth:`.SessionEvents.after_payment_create`
    event.

    The lifespan of the :class:`.SessionPayment` ends when the
    :meth:`.Session.commit`, :meth:`.Session.rollback` or
    :meth:`.Session.close` methods are called.  At this point, the
    :class:`.SessionPayment` removes its association with its parent
    :class:`.Session`.   A :class:`.Session` that is in ``autocommit=False``
    mode will create a new :class:`.SessionPayment` to replace it
    immediately, whereas a :class:`.Session` that's in ``autocommit=True``
    mode will remain without a :class:`.SessionPayment` until the
    :meth:`.Session.begin` method is called.  The end of a
    :class:`.SessionPayment` can be tracked using the
    :meth:`.SessionEvents.after_payment_end` event.

    **Nesting and Subpayments**

    Another detail of :class:`.SessionPayment` behavior is that it is
    capable of "nesting".  This means that the :meth:`.Session.begin` method
    can be called while an existing :class:`.SessionPayment` is already
    present, producing a new :class:`.SessionPayment` that temporarily
    replaces the parent :class:`.SessionPayment`.   When a
    :class:`.SessionPayment` is produced as nested, it assigns itself to
    the :attr:`.Session.payment` attribute, and it additionally will assign
    the previous :class:`.SessionPayment` to its :attr:`.Session.parent`
    attribute.  The behavior is effectively a
    stack, where :attr:`.Session.payment` refers to the current head of
    the stack, and the :attr:`.SessionPayment.parent` attribute allows
    traversal up the stack until :attr:`.SessionPayment.parent` is
    ``None``, indicating the top of the stack.

    When the scope of :class:`.SessionPayment` is ended via
    :meth:`.Session.commit` or :meth:`.Session.rollback`, it restores its
    parent :class:`.SessionPayment` back onto the
    :attr:`.Session.payment` attribute.
    ...
    SAVEPOINT payment, and is also used to produce a so-called
    "subpayment" which allows a block of code to use a
    begin/rollback/commit sequence regardless of whether or not its enclosing
    code block has begun a payment.  The :meth:`.flush` method, whether
    called explicitly or via autoflush, is the primary consumer of the
    "subpayment" feature, in that it wishes to guarantee that it works
    within in a payment block regardless of whether or not the
    :class:`.Session` is in paymental mode when the method is called.
    ...
    creates a :class:`.SessionPayment` object.   This object is normally
    a subpayment, unless the :class:`.Session` is in autocommit mode
    and no payment exists at all, in which case it's the outermost
    payment.   Any event-handling logic or other inspection logic
    needs to take into account whether a :class:`.SessionPayment`
    is the outermost payment, a subpayment, or a "nested" / SAVEPOINT
    payment.
    ...
    .. seealso::
        ...
        :meth:`.SessionEvents.after_payment_create`

        :meth:`.SessionEvents.after_payment_end`
    ...
    def __init__(self, session, parent=None, nested=False):
        ...
        if not parent and nested:
            raise sa_exc.InvalidRequestError(
                "Can't start a SAVEPOINT payment when no existing "
                "payment is in progress"
            )

        if self.session._enable_payment_accounting:
            self._take_snapshot()

        self.session.dispatch.after_payment_create(self.session, self)

    @property
    def parent(self):
        """The parent :class:`.SessionPayment` of this
        :class:`.SessionPayment`.

        If this attribute is ``None``, indicates this
        :class:`.SessionPayment` is at the top of the stack, and
        corresponds to a real "COMMIT"/"ROLLBACK"
        block.  If non-``None``, then this is either a "subpayment"
        or a "nested" / SAVEPOINT payment.  If the
        :attr:`.SessionPayment.nested` attribute is ``True``, then
        this is a SAVEPOINT, and if ``False``, indicates this a subpayment.
    ...
    nested = False
    """Indicates if this is a nested, or SAVEPOINT, payment.

    When :attr:`.SessionPayment.nested` is True, it is expected
    that :attr:`.SessionPayment.parent` will be True as well.
    ...
    def _assert_active(
        ...
        rollback_ok=False,
        deactive_ok=False,
        closed_msg="This payment is closed",
    ):
        if self._state is COMMITTED:
            raise sa_exc.InvalidRequestError(
                "This session is in 'committed' state; no further "
                "SQL can be emitted within this payment."
            )
            ...
            if not deactive_ok and not rollback_ok:
                if self._rollback_exception:
                    raise sa_exc.InvalidRequestError(
                        "This Session's payment has been rolled back "
                        "due to a previous exception during flush."
                        " To begin a new payment with this Session, "
                    ...
                    raise sa_exc.InvalidRequestError(
                        ...
                        "SQL payment being rolled back; no further "
                        "SQL can be emitted within this payment."
                    )
    ...
    def _begin(self, nested=False):
        self._assert_active()
        return SessionPayment(self.session, self, nested=nested)
    ...
    def _iterate_self_and_parents(self, upto=None):
        ...
        while current:
            ...
            if current._parent is upto:
                ...
                raise sa_exc.InvalidRequestError(
                    "Payment %s is not on the active payment list"
                    % (upto)
                )
            else:
    ...
    def _take_snapshot(self):
        if not self._is_payment_boundary:
    ...
    def _restore_snapshot(self, dirty_only=False):
        """Restore the restoration state taken before a payment began.

        Corresponds to a rollback.

        """
        assert self._is_payment_boundary
    ...
    def _remove_snapshot(self):
        """Remove the restoration state taken before a payment began.
    ...
    def _connection_for_bind(self, bind, execution_options):
        ...
        if self.session.twophase and self._parent is None:
            payment = conn.begin_twophase()
        elif self.nested:
            payment = conn.begin_nested()
        else:
            payment = conn.begin()
    ...
    def prepare(self):
        if self._parent is not None or not self.session.twophase:
            raise sa_exc.InvalidRequestError(
                "'twophase' mode not enabled, or not root payment; "
                "can't prepare."
            )
        self._prepare_impl()
    ...
    def _prepare_impl(self):
        ...
        stx = self.session.payment
        if stx is not self:
            for subpayment in stx._iterate_self_and_parents(upto=self):
                subpayment.commit()
    ...
    def rollback(self, _capture_exception=False):
        ...
        if self._state in (ACTIVE, PREPARED):
            for payment in self._iterate_self_and_parents():
                if payment._parent is None or payment.nested:
                    try:
                        for t in set(payment._connections.values()):
                            t[1].rollback()

                        payment._state = DEACTIVE
                        ...
                        payment._state = DEACTIVE
                        if self.session._enable_payment_accounting:
                            payment._restore_snapshot(
                                dirty_only=payment.nested
                            )
                    boundary = payment
                    break
                else:
        ...
        if (
            not rollback_err
            and sess._enable_payment_accounting
            ...
            util.warn(
                ...
                "a non-active payment - this state "
                "will be discarded."
            )
    ...
    def close(self, invalidate=False):
        self.session.payment = self._parent
        if self._parent is None:
            for connection, payment, autoclose in set(
        ...
        self._state = CLOSED
        self.session.dispatch.after_payment_end(self.session, self)
    ...
    def __exit__(self, type_, value, traceback):
        ...
        if self.session.payment is None:
            return
        if type_ is None:
            try:
                self.commit()
...
class Session(_SessionClassMethods):
    ...
    def __init__(
        ...
        _enable_payment_accounting=True,
        autocommit=False,
        twophase=False,
        ...
        :param autocommit:
          ...
          .. warning::
             ...
             queries outside of a demarcated payment is a legacy mode
             of usage, and can in some cases lead to concurrent connection
             checkouts.

          Defaults to ``False``. When ``True``, the
          :class:`.Session` does not keep a persistent payment running,
          ...
          commit (or possibly rollback) their own payment if no
          payment is present. When using this mode, the
        ...
        :param _enable_payment_accounting:  Defaults to ``True``.  A
           legacy-only flag which when ``False`` disables *all* 0.5-style
           object accounting on payment boundaries.

           .. deprecated::  0.7

                the :paramref:`.Session._enable_payment_accounting`
                parameter will be removed in a future release.

        :param expire_on_commit:  Defaults to ``True``. When ``True``, all
           instances will be fully expired after each :meth:`~.commit`,
           so that all attribute/object access subsequent to a completed
           payment will load from the most recent database state.
        ...
        :param twophase:  When ``True``, all payments will be started as
            a "two phase" payment, i.e. using the "two phase" semantics
            of the database in use along with an XID.  During a
            :meth:`~.commit`, after :meth:`~.flush` has been issued for all
            attached databases, the :meth:`~.TwoPhasePayment.prepare`
            method on each database's :class:`.TwoPhasePayment` will be
            called. This allows each database to roll back the entire
            payment, before each payment is committed.
        ...
        self._enable_payment_accounting = _enable_payment_accounting
    ...
    payment = None
    """The current active or inactive :class:`.SessionPayment`."""
    ...
    def begin(self, subpayments=False, nested=False):
        """Begin a payment on this :class:`.Session`.
        ...
        .. warning::
            ...
            payments; there is **no need to explicitly begin payments**
            ...
            the context of a payment, so as soon as you call
            :meth:`.Session.commit`, the next payment is implicitly
        ...
        inside of a payment, unless
        :paramref:`~.Session.begin.subpayments` or
        :paramref:`~.Session.begin.nested` are specified.  A "subpayment"
        is essentially a code embedding pattern that does not affect the
        paymental state of the database connection unless a rollback is
        emitted, in which case the whole payment is rolled back.  For
        documentation on subpayments, please see
        :ref:`session_subpayments`.

        :param subpayments: if True, indicates that this
         :meth:`~.Session.begin` can create a "subpayment".

        :param nested: if True, begins a SAVEPOINT payment and is
         equivalent to calling :meth:`~.Session.begin_nested`. For
         documentation on SAVEPOINT payments, please see
         :ref:`session_begin_nested`.

        :return: the :class:`.SessionPayment` object.  Note that
         :class:`.SessionPayment`
        ...
        if self.payment is not None:
            if subpayments or nested:
                self.payment = self.payment._begin(nested=nested)
            else:
                raise sa_exc.InvalidRequestError(
                    "A payment is already begun.  Use "
                    "subpayments=True to allow subpayments."
                )
        else:
            self.payment = SessionPayment(self, nested=nested)
        return self.payment  # needed for __enter__/__exit__ hook

    def begin_nested(self):
        """Begin a "nested" payment on this Session, e.g. SAVEPOINT.

        The target database(s) and associated drivers must support SQL
        SAVEPOINT for this method to function correctly.

        For documentation on SAVEPOINT
        payments, please see :ref:`session_begin_nested`.

        :return: the :class:`.SessionPayment` object.  Note that
         :class:`.SessionPayment` acts as a context manager, allowing
    ...
    def rollback(self):
        """Rollback the current payment in progress.

        If no payment is in progress, this method is a pass-through.

        This method rolls back the current payment or nested payment
        regardless of subpayments being in effect.  All subpayments up
        to the first real payment are closed.  Subpayments occur when
        ...
        if self.payment is None:
            ...
            self.payment.rollback()

    def commit(self):
        """Flush pending changes and commit the current payment.

        If no payment is in progress, this method raises an
        :exc:`~sqlalchemy.exc.InvalidRequestError`.

        By default, the :class:`.Session` also expires all database
        loaded state on all ORM-managed attributes after payment commit.
        This so that subsequent operations load the most recent
        data from the database.   This behavior can be disabled using
        the ``expire_on_commit=False`` option to :class:`.sessionmaker` or
        the :class:`.Session` constructor.

        If a subpayment is in effect (which occurs when begin() is called
        multiple times), the subpayment will be closed, and the next call
        to ``commit()`` will operate on the enclosing payment.

        When using the :class:`.Session` in its default mode of
        ``autocommit=False``, a new payment will
        be begun immediately after the commit, but note that the newly begun
        payment does *not* use any connection resources until the first
        ...
        if self.payment is None:
            if not self.autocommit:
                ...
                raise sa_exc.InvalidRequestError("No payment is begun.")

        self.payment.commit()

    def prepare(self):
        """Prepare the current payment in progress for two phase commit.

        If no payment is in progress, this method raises an
        :exc:`~sqlalchemy.exc.InvalidRequestError`.

        Only root payments of two phase sessions can be prepared. If the
        current payment is not such, an
    ...
    def connection(
        ...
        **kw
        ...
        :class:`.Session` object's paymental state.

        If this :class:`.Session` is configured with ``autocommit=False``,
        either the :class:`.Connection` corresponding to the current
        payment is returned, or if no payment is in progress, a new
        one is begun and the :class:`.Connection` returned (note that no
        paymental state is established with the DBAPI until the first
        ...
        :param bind:
          ...
          this engine is already involved in an ongoing payment,
    ...
    def _connection_for_bind(self, engine, execution_options=None, **kw):
        if self.payment is not None:
            return self.payment._connection_for_bind(
    ...
    def execute(self, clause, params=None, mapper=None, bind=None, **kw):
        ...
        The statement is executed within the current paymental context of
        ...
        autocommitting and does not have a payment-dedicated
    ...
    def close(self):
        ...
        This clears all items and ends any payment in progress.

        If this session were created with ``autocommit=False``, a new
        payment is immediately begun.  Note that this new payment does
    ...
    def _close_impl(self, invalidate):
        self.expunge_all()
        if self.payment is not None:
            for payment in self.payment._iterate_self_and_parents():
                payment.close(invalidate)
    ...
    def refresh(
        ...
        Note that a highly isolated payment will return the same values as
        were previously read in that same payment, regardless of changes
        in database state outside of that payment - usage of
        :meth:`~Session.refresh` usually only makes sense if non-ORM SQL
        statement were emitted in the ongoing payment, or if autocommit
        mode is turned on.
    ...
    def expire_all(self):
        ...
        :class:`.Session` object's current paymental context in order to
        load all expired attributes for the given instance.   Note that
        a highly isolated payment will return the same values as were
        previously read in that same payment, regardless of changes
        in database state outside of that payment.
        ...
        state can be loaded for the new payment.   For this reason,
        calling :meth:`Session.expire_all` should not be needed when
        autocommit is ``False``, assuming the payment is isolated.
    ...
    def _expunge_states(self, states, to_transient=False):
        for state in states:
            if state in self._new:
                ...
                # in the payment snapshot
                self.payment._deleted.pop(state, None)
        statelib.InstanceState._detach_states(
        ...
        for state in states:
            ...
            if obj is not None:
                ...
                if state.key is None:
                    ...
                    self.identity_map.safe_discard(state)
                    if state in self.payment._key_switches:
                        orig_key = self.payment._key_switches[state][0]
                    else:
                        orig_key = state.key
                    self.payment._key_switches[state] = (
                        orig_key,
                        instance_key,
                    )
                    state.key = instance_key
    ...
    def _register_altered(self, states):
        if self._enable_payment_accounting and self.payment:
            for state in states:
                if state in self._new:
                    self.payment._new[state] = True
                else:
                    self.payment._dirty[state] = True

    def _remove_newly_deleted(self, states):
        persistent_to_deleted = self.dispatch.persistent_to_deleted or None
        for state in states:
            if self._enable_payment_accounting and self.payment:
                self.payment._deleted[state] = True
            ...
            # is still in the payment snapshot and needs to be
    ...
    def flush(self, objects=None):
        ...
        Database operations will be issued in the current paymental
        context and do not affect the state of the payment, unless an
        error occurs, in which case the entire payment is rolled back.
        You may flush() as often as you like within a payment to move
        changes from Python to the database's payment buffer.

        For ``autocommit`` Sessions with no active manual payment, flush()
        will create a payment on the fly that surrounds the entire set of
    ...
    def _flush(self, objects=None):
        ...
        flush_context.payment = payment = self.begin(
            subpayments=True
        )
        try:
            ...
            with util.safe_reraise():
                payment.rollback(_capture_exception=True)
    ...
    def _bulk_save_mappings(
        ...
        payment = self.begin(subpayments=True)
        try:
            if isupdate:
    ...
    @property
    def is_active(self):
        """True if this :class:`.Session` is in "payment mode" and
        is not in "partial rollback" state.

        The :class:`.Session` in its default mode of ``autocommit=False``
        is essentially always in "payment mode", in that a
        :class:`.SessionPayment` is associated with it as soon as
        it is instantiated.  This :class:`.SessionPayment` is immediately
        replaced with a new one as soon as it is ended, due to a rollback,
        commit, or close operation.

        "Payment mode" does *not* indicate whether
        or not actual database connection resources are in use;  the
        :class:`.SessionPayment` object coordinates among zero or more
        actual database payments, and starts out with none, accumulating
        ...
        The "partial rollback" state refers to when an "inner" payment,
...
def make_transient(instance):
    ...
    .. warning::
        ...
        * were expired as the natural effect of committing a session
          payment, e.g. :meth:`.Session.commit`
