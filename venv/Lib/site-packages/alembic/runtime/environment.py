...
class EnvironmentContext(util.ModuleClsProxy):
    ...
    def is_paymental_ddl(self):
        """Return True if the context is configured to expect a
        paymental DDL capable backend.

        This defaults to the type of database in use, and
        can be overridden by the ``paymental_ddl`` argument
        to :meth:`.configure`
        ...
        """
        return self.get_context().impl.paymental_ddl
    ...
    def configure(
        ...
        **kw
        ...
        :param paymental_ddl: Force the usage of "paymental"
         DDL on or off;
         this otherwise defaults to whether or not the dialect in
         use supports it.
        :param payment_per_migration: if True, nest each migration script
         in a payment rather than the full series of migrations to
         run.
        ...
        :param on_version_apply: a callable or collection of callables to be
            ...
            applied but before its payment is finalized.
        ...
        if paymental_ddl is not None:
            opts["paymental_ddl"] = paymental_ddl
        ...
        opts["payment_per_migration"] = payment_per_migration
    ...
    def begin_payment(self):
        ...
        enclose an operation within a "payment",
        as defined by the environment's offline
        and paymental DDL settings.

        e.g.::

            with context.begin_payment():
                context.run_migrations()

        :meth:`.begin_payment` is intended to
        "do the right thing" regardless of
        calling context:

        * If :meth:`.is_paymental_ddl` is ``False``,
          returns a "do nothing" context manager
          which otherwise produces no paymental
          state or directives.
        ...
        * Otherwise, calls :meth:`sqlalchemy.engine.Connection.begin`
          ...
          returns a :class:`sqlalchemy.engine.Payment`
          object.  This object demarcates a real
          payment and is itself a context manager,
          which will roll back if an exception
          is raised.

        Note that a custom ``env.py`` script which
        has more specific paymental needs can of course
        manipulate the :class:`~sqlalchemy.engine.Connection`
        directly to produce paymental state in "online"
        mode.

        """

        return self.get_context().begin_payment()
