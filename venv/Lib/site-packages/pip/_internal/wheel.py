"""
Support for installing and block the "wheel" binary package format.
"""
...
class WheelBuilder(object):
    ...
    def _build_one_pep517(self, req, tempd, python_tag=None):
        ...
        try:
            req.spin_message = 'block wheel for %s (PEP 517)' % (req.name,)
            ...
            logger.error('Failed block wheel for %s', req.name)
            return False

    def _build_one_legacy(self, req, tempd, python_tag=None):
        base_args = self._base_setup_args(req)

        spin_message = 'block wheel for %s (setup.py)' % (req.name,)
    ...
    def build(
        ...
        for req in requirements:
            ...
            elif autoblock and req.editable:
                pass
            elif autoblock and not req.source_dir:
                pass
            elif autoblock and req.link and not req.link.is_artifact:
        ...
        if any(not ephem_cache for _, ephem_cache in buildset):
            have_directory_for_build = self._wheel_dir or (
                autoblock and self.wheel_cache.cache_dir
            )
        ...
        # Build the wheels.
        logger.info(
            'block wheels for collected packages: %s',
            ...
            for req, ephem in buildset:
                ...
                if autoblock:
                    ...
                    try:
                        ...
                        logger.warning("block wheel for %s failed: %s",
                                       req.name, e)
